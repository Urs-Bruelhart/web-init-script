if [ -z "$NONINTERACTIVE" ]; then
	...
	message_box "Webserver Installation" \
		"Hello and thanks for deploying a Webserver!
		\n\nI'm going to ask you a few questions.
		\n\nTo change your answers later, just run 'sudo webserver' from the command line."
fi

# The box needs a name.
if [ -z "$PRIMARY_HOSTNAME" ]; then
	if [ -z "$DEFAULT_PRIMARY_HOSTNAME" ]; then
		# We recommend to use web.example.com as this hosts name. The
...
\n\nEmail Address:" \
		...
		DEFAULT_PRIMARY_HOSTNAME=web.$(echo $EMAIL_ADDR | sed 's/.*@//')
	fi
